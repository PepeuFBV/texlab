...
| **Release** | Abre/atualiza um PR de release via release-please com CHANGELOG gerado a partir de Conventional Commits; ao merge cria um GitHub Release, faz upload do arquivo de templates (`templates-<version>.zip` / `.tar.gz`) e atualiza as tags flutuantes `vX` e `vX.Y` para apontar para a versão mais recente. | Push em `main` |
